...
export const Resources: React.FC<ResourcesProps> = ({ onSelectFeature }) => {
  ...
  const handlePaymentClick = (method: string) => {
    alert(`This is where the ${method} payment window would pop up for the ${selectedPlan} plan!`);
  };
    ...
    <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6">
      ...
      {selectedPlan && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-fade-in">
            <div className="bg-white dark:bg-slate-800 rounded-[3rem] p-10 max-w-lg w-full shadow-3xl border border-slate-200 dark:border-slate-700 relative overflow-hidden">
                <button onClick={() => setSelectedPlan(null)} className="absolute top-6 right-6 text-slate-400 hover:text-slate-600 dark:hover:text-white transition-colors">
                    <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
                
                <div className="text-center mb-8">
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-primary-600">Secure Checkout</span>
                    <h3 className="text-3xl font-black uppercase tracking-tighter mt-2">Finish Your Order</h3>
                    <p className="text-slate-500 font-medium mt-1">You picked the <strong>{selectedPlan}</strong> plan.</p>
                </div>

                <div className="space-y-4">
                    <button 
                        onClick={() => handlePaymentClick('PayPal')}
                        className="w-full h-16 bg-[#ffc439] hover:bg-[#f2ba36] rounded-2xl flex items-center justify-center gap-3 shadow-lg transition-transform active:scale-95 group"
                    >
                        <span className="font-bold text-[#27346a] text-lg italic">PayPal</span>
                    </button>
                    <button 
                        onClick={() => handlePaymentClick('Paystack')}
                        className="w-full h-16 bg-[#011b33] hover:bg-[#000e1a] rounded-2xl flex items-center justify-center gap-3 shadow-lg transition-transform active:scale-95 group"
                    >
                        <span className="font-black text-white text-lg tracking-tight uppercase">Pay with Paystack</span>
                    </button>
                </div>
            </div>
        </div>
      ...
      <section className="mb-48" id="pricing">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-[1400px] mx-auto items-stretch">
            ...
            <PricingTier 
                ...
                cta="Go Hunter" onCta={() => setSelectedPlan('Hunter')}
            />
            <PricingTier 
                ...
                cta="Get full access" onCta={() => setSelectedPlan('Authority')}
            />
            <PricingTier 
