...
export const JobSearchTool: React.FC<{ onBack: () => void }> = ({ onBack }) => {
          ...
          <form onSubmit={handleSearch} className="lg:col-span-2 space-y-8">
            ...
            <Button type="submit" disabled={isLoading} className="w-full h-16 text-lg font-black uppercase tracking-widest shadow-2xl rounded-2xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 hover:bg-slate-800 dark:hover:bg-slate-100 transform hover:-translate-y-1 transition-all flex items-center justify-center gap-3 group">
              {isLoading ? <Spinner size="sm" className="text-white dark:text-slate-900" /> : (
                <>
                  <span>Start my search</span>
                  <svg className="h-5 w-5 group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                </>
              )}
            </Button>
          </form>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-6 mb-8">
                    ...
                    <div className="flex flex-col items-end gap-3">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${savedJobIds.has(job.url)
